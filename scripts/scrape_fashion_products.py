...
def scrape_and_index_products(target_count: int = 500):
    ...
    try:
        ...
        for category in CATEGORIES:
            ...
            # Search Google Shopping - get multiple products per category
            products = search_google_shopping(
                query=f"{category}",
                num_results=30  # Get 30 products per category (17 categories × 30 ≈ 500)
            )

            # Process products from this category
            if not products:
